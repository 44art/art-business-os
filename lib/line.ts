...
function buildDeliveryNotes(goal?: string): string {
  const base = [
    ...
    '・長い場合は「続きはこちら↓」でLP・ブログに誘導',
    '',
    '【ブロック対策】',
    '・一方的な告知だけでなく、質問・呼びかけを入れる',
    '・返信したら必ず返す（コミュニケーションを大切に）',
    '・価値ある情報を継続的に届ける（宣伝だけにならない）',
  ]

  if (goal === 'repeat') {
    base.push('', '【リピーター向け注意点】', '・購入・参加後の感想フォローを必ず入れる', '・次回への橋渡しを自然な流れで行う')
  } else if (goal === 'fan') {
    base.push('', '【ファン向け注意点】', '・本音・裏話をシェアして特別感を出す', '・返信に丁寧に対応して関係を深める')
  }
