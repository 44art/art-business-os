...
function genArtworkSales(p: GenParams): Record<string, string> {
  ...
  return {
    ...
    product_appeal: [
      ...
      `• ジャンル：${genre}`,
      `• 価格：${price}`,
      `• 世界に一点のみ — 同じ作品は二度と制作しません`,
      `• 購入後のアフターフォローあり（ご不明点はDMでご連絡を）`,
      '',
      '▼ こんな方に特におすすめ',
      `• ${targetCustomer}`,
      `• 大切な方へ特別なプレゼントを贈りたい方`,
...
function genWsBooking(p: GenParams): Record<string, string> {
  ...
  return {
    ...
    product_appeal: [
      ...
      `• 参加費：${price}`,
      `• 所要時間：${duration}`,
      w?.capacity ? `• 定員：${w.capacity}名（先着順・少人数制）` : '• 少人数制（定員あり）',
      w?.location ? `• 場所：${w.location}` : '',
      `• 持ち物：（必要なものを記入してください）`,
      `• 初心者・未経験者歓迎 — 道具はすべてご用意します`,
      '',
      '▼ このWSで得られること',
      `• ${p.desire1}を体験できる`,
      '• 参加後に持ち帰れる・飾れる作品が完成する',
      '• 達成感と「自分で作った」という特別な満足感',
      '• 同じ趣味を持つ方とのつながり',
    ].filter(Boolean).join('\n'),
    trust: [
      '▼ 参加者の声',
